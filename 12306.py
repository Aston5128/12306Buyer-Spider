...
class crawl12306:
    def __init__(self):
        ...
        self.from_station = re.findall(from_station_CHS + '\|(.*?)\|', self.city)[0]  # 将出发地改为相应的城市代码
        self.to_station = re.findall(to_station_CHS + '\|(.*?)\|', self.city)[0]  # 将目的地地改为相应的城市代码
    ...
    def getData(self):
        html = requests.get('https://kyfw.12306.cn/otn/leftTicket/query?leftTicketDTO.train_date=' + self.departure_date
                            + '&leftTicketDTO.from_station=' + self.from_station + '&leftTicketDTO.to_station='
                            + self.to_station + "&purpose_codes=ADULT", verify=False).text
